...
class MainActivity : ComponentActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContent {
            ...
            connectNode(mContext)
        }
    }
}

fun connectNode(context: Context) {
    // [START write_message]
    // Write a message to the database
    val database = Firebase.database
    val myRef = database.getReference("fall_detect_system")

    // [END write_message]

    // [START read_message]
    // Read from the database
    myRef.child("signal").addValueEventListener(object : ValueEventListener {
        override fun onDataChange(dataSnapshot: DataSnapshot) {
            ...
            if (value == "1"){
                makePhoneCall("913171996",context)
            }
